...
The cache doubles as the lifecycle's observability seam. `getConnectedDBs` exports the map as a list of `{ projectDir, openedAt, lastAccessed }` records (`src/server/index.ts:53-60`), and that function is handed to the server-info tools at registration so a caller can ask which projects this process currently holds open (`src/tools/index.ts:54`, `src/tools/server-info-tools.ts:60-61`). A maintainer adding lifecycle telemetry — idle timeouts, connection limits, eviction — should start from `dbMap` and `getConnectedDBs`, because they are the only place the set of live connections is materialized.
...
- **Add a tool to the running server:** add a `registerX` import and one call in `registerAllTools` (`src/tools/index.ts:39-56`). The handler receives `getDB` (and optionally `writeStatus`) for free; it must not import `src/server/index.ts`.
...
- `src/indexing/watcher.ts` — `startWatcher`, the debounced recursive file watcher started after the first full index completes.
...
- `src/utils/index-lock.ts` — `tryAcquireIndexLock`, the per-project lock that elects the single indexing instance and reclaims stale locks.
