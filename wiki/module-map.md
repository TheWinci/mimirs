...
`src/graph` owns the meaning of imports and symbol references, and it now holds two distinct modules that both read the graph tables back through `RagDB`. The first is `resolver.ts`: it resolves an import specifier to a concrete file in two passes — `@winci/bun-chunk`'s filesystem resolver first (which understands tsconfig paths, Python, and Rust), then a fallback that probes the database's own indexed file paths with the known extensions (`src/graph/resolver.ts:24-61`). The same file also builds the project dependency map that `project_map` and the [mimirs map](cli/map.md) command print, grouping edges by file or directory (`src/graph/resolver.ts:181-225`). The indexer calls the resolver while indexing; the graph store in `src/db/graph.ts` persists the resolved edges; and `project_map`, `depends_on`, `dependents`, and the search graph-boost all read them back.

The second module is `trace.ts`, the symbol-level call-graph walker. Where the resolver works at file granularity, `trace.ts` works at callable granularity: it reads the forward and reverse call edges the database already resolved (`getCalleeRefsForExport`, `getCallersOfExport`) and turns them into two questions an agent actually asks — `impactWalk` returns the transitive callers of a symbol as a pruned tree (blast radius), and `tracePath` returns the reachable sub-graph of every call path between two symbols (`src/graph/trace.ts:224-310`, `src/graph/trace.ts:364-444`). It is bounded on purpose: a callable with more than `AMBIENT_FANIN` (25) distinct callers is cited rather than expanded so a hot utility cannot explode the walk (`src/graph/trace.ts:37-77`), and the count pass is capped at `COUNT_CAP` (2000) so a pathological symbol reports `≥N` instead of running unbounded (`src/graph/trace.ts:222`). The `impact` and `trace` MCP tools consume these from `src/tools/graph-tools.ts:242`, and the file-level `transitiveImporters`/`affectedTests` helpers in the same module back the `affected` CLI command dispatched from `src/cli/index.ts:140-142` (`src/graph/trace.ts:488-571`). Resolution is static name-match, so a dynamic-dispatch hop (callback, interface→impl, DI) ends a chain — a limit the tool output states to the caller rather than hiding.
...
The directories form a layered graph with no cycles across layers. The front doors (`src/cli`, `src/server`) and the tool registry (`src/tools`) depend downward on the core. The core directories — `indexing`, `search`, `graph`, `conversation`, `git`, `wiki` — depend on `src/db` and `src/embeddings`, and they avoid reaching into each other's internals with two narrow, deliberate exceptions. Search consults the graph tables through `RagDB` for its graph-boost, and indexing imports the graph resolver directly — `src/indexing/indexer.ts` and `src/indexing/watcher.ts` both call `resolveImports`/`resolveImportsForFile` from `src/graph/resolver.ts` so that import edges are resolved as part of indexing rather than in a separate pass. Both exceptions point the same direction (toward `graph`), so no cycle forms. `src/db` depends on `src/config` and `src/embeddings`. `src/config` depends only on `src/embeddings`. `src/embeddings` and `src/utils` depend on nothing internal.
