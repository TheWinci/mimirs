...
| `indexBatchSize` | int ≥ 1, optional | `50` | Files/chunks embedded per batch during indexing (`src/indexing/indexer.ts:390`, `src/indexing/indexer.ts:560`). Falls back to `50` when unset. |
| `indexThreads` | int ≥ 1, optional | unset | Thread count handed to the embedder (`src/indexing/indexer.ts:474`); when unset the embedder picks a default from CPU count (`src/embeddings/embed.ts:27-29`). |
...
| `RAG_DB_DIR` | `<projectDir>/.mimirs` | `src/db/index.ts:101-105` | Where the SQLite index lives. Override it when the project directory is read-only; the error path for a write failure points the user straight at this variable (`src/db/index.ts:112-120`). |
...
The default model is `Xenova/all-MiniLM-L6-v2` at 384 dimensions, both declared as constants in the embedder module (`src/embeddings/embed.ts:16-17`). The embedder keeps the active model id and dimension in module-level singletons; `configureEmbedder(modelId, dim)` is the only way to change them, and it clears the cached pipeline and tokenizer when the model or dim actually changes (`src/embeddings/embed.ts:35-42`). Everything that produces a vector reads the current singletons through `getEmbeddingDim` and `getModelId` (`src/embeddings/embed.ts:201-209`).
