...
The command is wired into the CLI dispatcher under the `read` case, which forwards the raw arguments and a flag-lookup helper to the handler `readCommand` `src/cli/index.ts:125-126`. That handler lives next to the file-level `search` handler in the same module `src/cli/commands/search-cmd.ts:61-88`.
...
5. `searchChunks` embeds the query text into a vector via `embed` `src/search/hybrid.ts:500-501`.
6. It runs a vector (nearest-neighbour) search and an identifier-aware BM25 full-text search over the chunk index, each over-fetching `topK * 4` candidates `src/search/hybrid.ts:503-510`.
7. It fuses the two result lists by reciprocal-rank fusion, drops anything below the threshold, applies path and filename boosts, demotes boilerplate/generated files, adds a dependency-graph boost, sorts, then consolidates sibling chunks into their parent and expands for docs `src/search/hybrid.ts:512-562`.
8. Before returning, it writes one row into the `query_log` table for analytics `src/search/hybrid.ts:566-573`.
...
| `--top N` | integer | no | Number of chunks to return. Parsed with a strict integer parser, minimum 1; **defaults to 8** (a literal, not the config `searchTopK`) `src/cli/commands/search-cmd.ts:71`. |
| `--threshold T` | float 0–1 | no | Minimum fused score a chunk must reach to be kept. Defaults to `0.3` `src/cli/commands/search-cmd.ts:72`. |
...
| `query_log` row | One row inserted into the SQLite `query_log` table, side-effect of every chunk search `src/search/hybrid.ts:566-573`. |
...
- **Dependency-graph boost.** A chunk in a file that many others import gets a small additive logarithmic bonus (`0.05 * log2(importerCount + 1)`), so widely-used code surfaces higher `src/search/hybrid.ts:546-554`.
- **Parent grouping.** When `parentGroupingMinCount` or more sibling chunks share the same parent, they are replaced by the single parent chunk (keeping the best score) so a class does not flood the list with its individual methods `src/search/hybrid.ts:424-484,559`. This is why a result may have `chunkIndex` `-1` and represent a whole parent rather than one method `src/search/hybrid.ts:465`.
- **Doc expansion.** If markdown results are displacing code within the top `topK`, the list is extended slightly so docs are treated as bonus results instead of crowding out code `src/search/hybrid.ts:304-315,562`.
...
Every call to `searchChunks` ends by calling `db.logQuery(...)` with the query string, the number of results, the score and path of the top result, and the elapsed milliseconds measured from a `performance.now()` timer started at the top of the function `src/search/hybrid.ts:566-573`. One detail matters for analytics: the score logged is *not* the displayed fused score but `vectorResults[0]?.score` — the raw top vector cosine. Because the fused score is a positional rank value (close to `1` at the top regardless of true relevance), the raw cosine is logged instead so that "average top score" and the low-relevance (`< 0.3`) heuristic stay meaningful `src/search/hybrid.ts:564-571`. The path is the top *result's* path, or `null` when the result set is empty.

`db.logQuery` delegates to the analytics layer, which runs a single `INSERT INTO query_log (...)` with the current ISO timestamp `src/db/index.ts:949-954`, `src/db/analytics.ts:3-8`. The table is created at database open time with columns `query`, `result_count`, `top_score`, `top_path`, `duration_ms`, and `created_at` `src/db/index.ts:343-351`.
...
| Full-text search failure | Falls back to vector-only results after a debug log; the command still returns whatever the vector search found `src/search/hybrid.ts:505-510`. |
...
| File dedup | None — multiple chunks per file allowed `src/search/hybrid.ts:486-488` | Deduplicated to best chunk per file `src/search/hybrid.ts:356-376` |
| Symbol expansion | No extra symbol merge step | Adds exact symbol-name hits into the candidate pool `src/search/hybrid.ts:378-391` |
...
- `src/db/search.ts` — `textSearchChunks` (identifier-aware BM25 over `fts_chunks`) and `vectorSearchChunks`.
- `src/indexing/identifiers.ts` — `identifierParts` / `splitIdentifier` that fill the FTS `parts` column.
- `src/search/usages.ts` — `sanitizeFTS`, which quotes and OR-joins query tokens for FTS5.
...
- `src/db/index.ts` — `RagDB` wrappers (`searchChunks`, `textSearchChunks`, `logQuery`) and the `query_log` schema.
