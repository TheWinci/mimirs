...
— `src/search/hybrid.ts:421-428`. A second search variant records the same shape
after parent-grouping and doc expansion — `src/search/hybrid.ts:688-694`. Both
call the database wrapper `db.logQuery(...)`, which inserts into the `query_log`
table — `src/db/analytics.ts:3-8`.

The `top_score` that gets logged is **not** the score shown to the caller. The
visible result score is a positional rank-fusion value that sits near `1.0` at
the top regardless of true relevance, so logging it would make the
average-top-score and the `< 0.3` low-relevance heuristic meaningless. Both call
sites instead log the top vector hit converted back to **true cosine
similarity** — `vectorScoreToCosine(vectorResults[0]?.score)`, which is `null`
when there were no vector hits — `src/search/hybrid.ts:425`,
`src/search/hybrid.ts:692`. The stored vector score is L2-distance-based and
bottoms out near `0.33`, so this conversion is what keeps the `< 0.3`
low-relevance heuristic from being permanently dead — `src/db/search.ts:9-26`.
That cosine value is the relevance signal this report aggregates into `Avg top
score` and the low-relevance list.

The table is created on database open if it does not already exist —
`src/db/index.ts:482-490`:
...
| `top_score` | REAL (nullable) | Top vector hit's cosine similarity for the query, or NULL when there were no vector hits |
...
sequenceDiagram
    ...
    Handler->>Store: new RagDB(dir) (opens index.db)
    Handler->>Store: getAnalytics(days)
    Store->>Log: 7 aggregate SELECTs since now - days
    Log-->>Store: totals, averages, top/zero/low lists, per-day counts
    Store-->>Handler: analytics object
    Handler->>Handler: compute zero-result rate, print summary + lists
    Handler->>Store: getAnalyticsTrend(days)
    Store->>Log: counts for current window and prior window
    Log-->>Store: current / previous / delta
    Store-->>Handler: trend object
...
1. The user runs `mimirs analytics`, optionally with a directory and a
   ...
   calls the handler — `src/cli/index.ts:138-139`.
...
4. It opens the index database for that directory by constructing a `RagDB`,
   ...
   schema (including `query_log`) exists — `src/db/index.ts:147`,
...
5. It calls `getAnalytics(days)`, which runs seven aggregate SQL queries over
   ...
   `src/db/analytics.ts:10-77`.
6. The handler computes the zero-result rate from the returned true count and
   prints the summary block, then prints the three optional lists when they
   are non-empty — `src/cli/commands/analytics.ts:14-44`.
7. It calls `getAnalyticsTrend(days)` to compare the current window against the
   ...
   `src/cli/commands/analytics.ts:47`, `src/db/analytics.ts:79`.
8. If either window saw any queries, it prints the trend block with signed
   deltas — `src/cli/commands/analytics.ts:48-59`.
9. It closes the database and returns. All output has already gone to stdout
   through the CLI logger — `src/cli/commands/analytics.ts:61`.
...
number of milliseconds in a day — `src/db/analytics.ts:22`. Every query in this
...
`getAnalytics` runs seven reads over that window — `src/db/analytics.ts:24-65`.
...
| `zeroResultCount` | `COUNT(*)` of all rows where `result_count = 0` — the true count, not a sum of the display list | (drives the rate) |
...
The `queriesPerDay` series is part of the returned object — `src/db/analytics.ts:61-65`
...
The handler always prints a four-line summary, then up to three optional list
sections, then an optional trend block. Output goes through the CLI logger,
which writes plain lines to stdout via `console.log` with no prefix —
`src/utils/log.ts:51-55`.
...
zero-result rate — `src/cli/commands/analytics.ts:19-23`.

The zero-result rate is computed from the **true** zero-result count, not from
the display list. The handler reads `analytics.zeroResultCount` — a direct
`COUNT(*)` over `result_count = 0` — divides it by `totalQueries`, and rounds to
a whole percent. When there were no queries at all, it short-circuits to `"0"`
to avoid dividing by zero — `src/cli/commands/analytics.ts:14-17`. This matters:
the `zeroResultQueries` list shown below is capped at the 10 most frequent
zero-result queries, so summing *its* counts would understate the rate on a
diverse log. The rate and its `(N queries)` count both come from the full count,
while the list underneath is only the top ten — `src/db/analytics.ts:42-46`.
...
- **Top searches** — the most frequent query strings, shown as `count× "query"`
  — `src/cli/commands/analytics.ts:25-30`.
- **Zero-result queries** — frequent searches that returned nothing, labeled as
  topics worth indexing — `src/cli/commands/analytics.ts:32-37`.
- **Low-relevance queries** — searches whose best match scored below `0.3`,
  shown with that score to two decimals — `src/cli/commands/analytics.ts:39-44`.
...
for the previous window — and counts queries in each — `src/db/analytics.ts:84-114`.
...
bounded by a `since` and an `until` timestamp — `src/db/analytics.ts:88-110`.
...
block is skipped — `src/cli/commands/analytics.ts:48`. When it does print, two
...
to one decimal — `src/cli/commands/analytics.ts:49-51`. The average-top-score
...
change — `src/cli/commands/analytics.ts:55-57`.
...
`src/db/analytics.ts:10`, `src/db/analytics.ts:79`. The command never relies on
...
`src/cli/commands/analytics.ts:10`, `src/cli/commands/analytics.ts:47`.
...
| --- | --- |
| Usage summary | Four lines on stdout: total queries, average results, average top score, and zero-result rate for the window — `src/cli/commands/analytics.ts:19-23`. |
| Top searches list | Optional block listing the most frequent queries with counts — `src/cli/commands/analytics.ts:25-30`. |
| Zero-result list | Optional block of frequent searches that returned nothing — `src/cli/commands/analytics.ts:32-37`. |
| Low-relevance list | Optional block of searches whose best score was under `0.3` — `src/cli/commands/analytics.ts:39-44`. |
| Trend block | Optional block comparing the current window to the prior equal-length window, with signed deltas — `src/cli/commands/analytics.ts:53-58`. |
...
- **No directory argument** — falls back to the current directory; the
  ...
  `src/cli/commands/analytics.ts:7`, `src/cli/flags.ts:63-66`.
- **Invalid `--days`** — a non-integer such as `--days abc`, or a value below
  ...
  date math — `src/cli/flags.ts:40-53`, `src/cli/index.ts:105-108`.
- **Empty log / brand-new index** — `totalQueries` is `0`, `avgResultCount`
  ...
  zero — `src/cli/commands/analytics.ts:15-17`, `src/db/analytics.ts:68-71`.
- **No queries in either trend window** — the entire trend block is skipped
  — `src/cli/commands/analytics.ts:48`.
- **No scored queries in a window** — `avgTopScore` is `null` for that window,
  ...
  the average score — `src/cli/commands/analytics.ts:22`,
  `src/cli/commands/analytics.ts:55`, `src/db/analytics.ts:116-121`.
- **Empty list sections** — top searches, zero-result, and low-relevance
  ...
  index prints just the summary — `src/cli/commands/analytics.ts:25`,
  `src/cli/commands/analytics.ts:32`, `src/cli/commands/analytics.ts:39`.
- **List truncation** — each of the three lists is capped at 10 rows by the
  ...
  `src/db/analytics.ts:38`, `src/db/analytics.ts:50`, `src/db/analytics.ts:57`.
...
- `src/db/search.ts` — `vectorScoreToCosine`, which turns the stored vector
  score into the true cosine used as `top_score`.
