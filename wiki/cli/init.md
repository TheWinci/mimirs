...
`mimirs init` is the onboarding command. It prepares a project so that an AI coding agent can use the mimirs RAG tools: it writes the local config, registers the MCP server in the agent's config files, drops tool-usage instructions where the agent will read them, keeps the index out of version control, and then offers to build the search index immediately. It is the first command most users run in a repository, and it is safe to re-run — every step it performs is idempotent and only reports what actually changed.

The command is reached through the top-level dispatcher. When the first argv token is `init`, the dispatcher forwards the raw argument array and a `getFlag` helper to `initCommand` (`src/cli/index.ts:112-113`). All of the real work lives in `src/cli/commands/init.ts` and the setup helpers in `src/cli/setup.ts`.

## What problem it solves

Wiring an MCP server into an agent by hand means editing several JSON and Markdown files in formats that differ per IDE (Claude Code, Cursor, Windsurf, JetBrains/Junie, GitHub Copilot), getting the server command and the project-directory environment variable exactly right, and remembering to gitignore the index. `init` does all of that in one pass and prints a line for each file it touched, so the user can see exactly what was created or updated.
...
sequenceDiagram
    ...
    Setup->>FS: ensureConfig / ensureAgentInstructions / ensureMcpJson / ensureGitignore
...
2. `initCommand` resolves the target directory and reads its flags. The directory is the first positional argument if present and not a flag, otherwise the current directory; `--yes`/`-y` and `--verbose`/`-v` are detected by membership in the argv array (`src/cli/commands/init.ts:11-13`).
3. The `--ide` value (if any) is parsed into a list of IDE names by `parseIdeFlag`, then passed to `runSetup` (`src/cli/commands/init.ts:14-16`).
4. `runSetup` performs the file-writing steps and returns the list of human-readable actions plus any IDE names it did not recognize (`src/cli/setup.ts:324-340`).
...
6. When the user passed IDE names that mimirs cannot configure automatically, the command prints a ready-to-paste MCP JSON snippet so the user can wire those agents up by hand (`src/cli/commands/init.ts:23-26`).
...
8. If indexing is accepted, the command opens the database, loads config, and runs `indexDirectory`, forwarding progress both to the terminal and to a `.mimirs/status` file (`src/cli/commands/init.ts:30-77`).
...
| `--ide` | comma list or `all` | no | Which agents to configure beyond Claude Code. `parseIdeFlag` lowercases and splits on commas, and expands the literal `all` to every known IDE (`src/cli/setup.ts:157-160`). Recognized values: `claude`, `cursor`, `windsurf`, `copilot`, `jetbrains` (`src/cli/setup.ts:148-149`). |
| `-y` / `--yes` | flag | no | Skips the indexing prompt and indexes immediately (`src/cli/commands/init.ts:12`, `:29`). |
| `-v` / `--verbose` | flag | no | Switches the indexing output from a single updating progress line to per-file logging (`src/cli/commands/init.ts:13`, `:62-76`). |

## Outputs

| output | where it lands / shape / description |
| --- | --- |
| `.mimirs/config.json` | Written by `ensureConfig` the first time, which calls `loadConfig` to materialize the defaults on disk (`src/cli/setup.ts:92-98`, `src/config/index.ts:136-139`). |
| Agent instruction blocks | A `## Using mimirs tools` Markdown section appended to `CLAUDE.md` (always), plus rule files for any requested or detected IDE: `.cursor/rules/mimirs.mdc`, `.windsurf/rules/mimirs.md`, `.junie/guidelines/mimirs.md`, `.github/copilot-instructions.md` (`src/cli/setup.ts:162-215`). |
...
| `.gitignore` entry | `.mimirs/` added (or the file created) so the local index is not committed (`src/cli/setup.ts:100-112`). |
...
`parseIdeFlag` accepts any string, so a typo or an unsupported agent name flows through unfiltered. `runSetup` runs the value through `unknownIdes`, which returns every name not in the known set (`src/cli/setup.ts:151-155`). When that list is non-empty, `initCommand` cannot configure those agents automatically, so it prints the ready-to-paste `mcpConfigSnippet` instead (`src/cli/commands/init.ts:23-26`). The snippet is the same `mimirs` server object used everywhere else — `bunx mimirs@latest serve` with `RAG_PROJECT_DIR` set to the absolute project path — formatted as indented JSON for the user to copy into whatever config their agent uses (`src/cli/setup.ts:217-226`).

## Optional index prompt and indexing

After setup, indexing is offered, not forced. `confirm` opens a readline interface, prints `Index project now? [Y/n] `, and resolves true for any answer except `n` — so a bare Enter means yes (`src/cli/setup.ts:314-322`). With `-y`, the prompt is skipped entirely (`src/cli/commands/init.ts:29`).

If indexing proceeds, the command constructs a `RagDB` for the directory (which opens the SQLite index under `.mimirs`) and loads the config, then calls `indexDirectory` (`src/cli/commands/init.ts:31-33`, `:49`). The progress callback does double duty:
...
- **Unknown IDE names.** Counts as a reason to print output even when no files changed, and triggers the manual MCP snippet (`src/cli/commands/init.ts:17`, `:23-26`).
...
- **IDE files when the directory is absent and not forced.** Cursor, Windsurf, Junie, and Copilot files are skipped unless their directory exists or the IDE was requested via `--ide` (`src/cli/setup.ts:171-212`, `:268-292`).
...
- `src/cli/index.ts` — top-level dispatcher; matches the `init` command and provides `getFlag` (`src/cli/index.ts:81-84`, `:112-113`).
...
- `src/cli/setup.ts` — the idempotent setup helpers: config, agent instructions, MCP registration, gitignore, IDE parsing, the snippet, and `confirm` (`src/cli/setup.ts`).
- `src/cli/progress.ts` — the quiet and verbose terminal progress renderers passed to the indexer (`src/cli/progress.ts`).
