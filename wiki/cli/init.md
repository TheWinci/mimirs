...
| MCP server entries | `mimirs` added under `mcpServers` in `.mcp.json` (always), and in `.cursor/mcp.json`, `.junie/mcp.json`, and the two Windsurf global configs when relevant — each running `bunx mimirs@^1 serve` with `RAG_PROJECT_DIR` set to the resolved project path. For GitHub Copilot the entry is written to `.vscode/mcp.json`, which uses a different shape: a top-level `servers` map and a `type: "stdio"` field (`src/cli/setup.ts:352-395`). |
...
- **MCP registration.** `ensureMcpJson` builds one server entry — `bunx mimirs@^1 serve` with `RAG_PROJECT_DIR` pointing at the resolved directory — and upserts it into each relevant config file (`src/cli/setup.ts:352-395`). `.mcp.json` is always written for Claude Code, with the entry under `mcpServers`. Cursor (`.cursor/mcp.json`) and Junie (`.junie/mcp.json`) are written when their directory exists or the IDE was requested; Windsurf targets two global paths under `~/.codeium` because it reads its MCP config from the user home, not the project (`src/cli/setup.ts:379-386`). GitHub Copilot is written to `.vscode/mcp.json` through a separate writer, `upsertVscodeMcp`, because VS Code uses a top-level `servers` map and a `type: "stdio"` field instead of `mcpServers` (`src/cli/setup.ts:332-350`, `src/cli/setup.ts:388-392`). Both `upsertMcpJson` and `upsertVscodeMcp` read and merge existing JSON, return `null` if the `mimirs` entry is already present and needs no migration, and report a skip line rather than crashing when the existing file is invalid JSON (`src/cli/setup.ts:308-328`).

- **Package spec pin.** The entry runs `mimirs@^1`, exported as `PACKAGE_SPEC`, not `mimirs@latest`. With `@latest`, publishing 2.0.0 would reach every existing installation on the next server start, so a breaking change would land with no opt-in. The major range keeps 1.x fixes automatic and makes the v2 jump an explicit act.

- **Migration of existing entries.** When an entry is already present, `migrateEntry` runs two independent rewrites and returns the reasons for each one it applied (`src/cli/setup.ts`). `upgradeBareBunx` replaces a bare `bunx` command with the absolute path. `upgradePackageSpec` replaces a `mimirs@latest` arg with `PACKAGE_SPEC`, in place, so any other arg or env var the user set survives. A version the user pinned themselves, such as `mimirs@1.6.0`, is not touched. When neither rewrite applies, the upsert returns `null` and writes nothing.
