...
3. If a `query` was supplied, the handler embeds it. `embed` runs the configured sentence-embedding model (the default is `Xenova/all-MiniLM-L6-v2`, a 384-dimension model) over the query text and returns one mean-pooled, normalized `Float32Array` (`src/embeddings/embed.ts:78-86`).
