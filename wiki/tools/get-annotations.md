...
3. If a `query` was supplied, the handler embeds it. `embed` runs the configured sentence-embedding model (the default is `Xenova/all-MiniLM-L6-v2`, a 384-dimension model) over the query text and returns one mean-pooled, normalized `Float32Array` (`src/embeddings/embed.ts:95-103`).
...
| `directory` | string | no | Which project to read from. Defaults to the `RAG_PROJECT_DIR` environment variable, then the current working directory. Resolved to an absolute path and checked for existence by `resolveProject` (`src/tools/index.ts:26-34`). |
...
- **Missing or invalid directory** — `resolveProject` throws `Directory does not exist: <path>` when the resolved directory is absent on disk, so the tool surfaces an error instead of querying (`src/tools/index.ts:32-34`).
