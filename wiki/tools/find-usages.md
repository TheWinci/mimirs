...
sequenceDiagram
    ...
    alt no results
        ...
        Tool-->>Caller: count line + per-file rows + depended_on_by tip
    end
```

1. The caller invokes the tool with a required `symbol`, an optional `exact` flag, an optional `top` limit, and an optional `directory`. The handler is registered inside `registerGraphTools` at `src/tools/graph-tools.ts:53-107`.
...
3. The handler calls `ragDb.findUsages(symbol, exact ?? true, top ?? 30)`. A missing `exact` defaults to `true` and a missing `top` defaults to `30` (`src/tools/graph-tools.ts:71`). The method on the database class just forwards to the query layer (`src/db/index.ts:667-669`).
...
With the default `exact: true`, a search for `getDB` returns uses of `getDB` but not `getDBForProject` — the reference name must be equal (case-insensitively), and the fallback regex requires a word boundary on both sides (`src/db/search.ts:442-454`, `src/db/search.ts:521`). A regression test confirms this: seeding a use of `getDBForProject` and searching `getDB` with `exact: true` does not return that file (`tests/search/find-usages.test.ts:66-87`).
...
| --- | --- | --- | --- |
| `symbol` | string (1–200 chars) | yes | The identifier to find references to. Matched case-insensitively against the reference index and, in the fallback, against chunk text (`src/tools/graph-tools.ts:57`). |
...
| `top` | integer (≥ 1) | no | Maximum number of usage rows to return. Defaults to `30`. Both lookup paths stop once this many results accumulate (`src/tools/graph-tools.ts:66`, `src/db/search.ts:486`, `src/db/search.ts:549`). |
...
- **No usages found.** When the result list is empty, the handler returns the `No usages of "<symbol>" found.` message and stops. This happens for a genuinely unused symbol, for a symbol that appears only in its own defining file (those rows are excluded), or when the index has not been built or refreshed (`src/tools/graph-tools.ts:73-77`). A test confirms an unknown symbol yields an empty array (`tests/search/find-usages.test.ts:99-103`).
...
- **Prefix matching via `exact: false`.** Setting `exact: false` widens both paths to prefix matches. A test seeds `getDB_cache` — which FTS5 tokenizes so that `getDB` is indexed — and confirms a non-exact `getDB` search surfaces it (`tests/search/find-usages.test.ts:89-97`).
...
- `src/db/graph.ts` — `upsertSymbolRefs` populates and maintains the `symbol_refs` reference index that the primary path reads (`src/db/graph.ts:11-26`).
- `src/search/usages.ts` — the `escapeRegex` helper used to build a literal word-boundary pattern (`src/search/usages.ts:14-16`).
- `src/db/index.ts` — defines the `symbol_refs` table and exposes the `findUsages` wrapper on the database class (`src/db/index.ts:248-260`, `src/db/index.ts:667-669`).
