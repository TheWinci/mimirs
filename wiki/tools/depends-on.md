...
The tool is registered inside `registerGraphTools`, alongside `project_map`, `find_usages`, and `depended_on_by`, against the shared MCP server `src/tools/graph-tools.ts:109`. The handler is small because the heavy work — parsing imports and resolving each specifier to a concrete file — already happened during indexing. At request time the handler does only a project lookup, a path lookup, and one query.
...
1. The MCP client invokes the tool with a `file` path and an optional `directory` `src/tools/graph-tools.ts:113`.
...
4. `getFileByPath` looks up that absolute path in the `files` table. The lookup runs the path through `normalizePath` first, which rewrites every backslash to a forward slash, so a Windows-style input still matches the stored POSIX-style path `src/db/files.ts:7`.
...
7. If the file imports nothing that resolved to another indexed file, the tool returns `<file> has no indexed dependencies.` `src/tools/graph-tools.ts:129`.
8. Otherwise it builds a header with the dependency count, then one indented line per dependency, converting each stored absolute path back to a project-relative path with `relative(projectDir, dep.path)` and appending the original import specifier `src/tools/graph-tools.ts:133`.
