...
- **Zero-result count** — a separate `COUNT(*)` of rows whose `result_count` was `0`. This is the *true* total, used for the rate, and is kept distinct from the display list below.
- **Top searched terms** — the ten most frequent query strings, grouped and ordered by count.
- **Zero-result queries** — the ten most frequent queries whose `result_count` was `0`, grouped by query and ordered by count. This is a `LIMIT 10` display list, not the rate basis.
...
sequenceDiagram
    ...
    Query->>Log: COUNT, AVG, zero-count, top terms,<br>zero-result, low-score (all >= since)
...
2. `resolveProject` turns the optional directory into an absolute path, verifies it exists, loads the project config, and returns the project's database handle (`src/tools/index.ts:33-83`).
3. The tool calls `getAnalytics(days)` on that database handle (`src/tools/analytics-tools.ts:25`).
4. The wrapper method forwards to the standalone analytics query function with the live SQLite connection (`src/db/index.ts:1244-1246`).
5. The function derives the window start as `Date.now() - days * 86400000` and converts it to an ISO string (`src/db/analytics.ts:22`).
6. It runs the count, average, zero-count, top-term, zero-result, and low-score queries — each restricted to rows whose `created_at` is at or after the window start.
7. SQLite returns the aggregated rows.
8. The function assembles them into one summary object and returns it up through the wrapper (`src/db/analytics.ts:67-76`).
...
| --- | --- | --- | --- |
| `directory` | string | no | Project directory to report on. When omitted, falls back to the `RAG_PROJECT_DIR` environment variable, then the current working directory (`src/tools/index.ts:38`). |
...
- **Zero-result rate** — computed in the tool from the true `zeroResultCount` divided by total queries, shown as a whole-number percent. When there were no queries at all it prints `0%` to avoid dividing by zero (`src/tools/analytics-tools.ts:32`).
...
The rate uses an exact count on purpose. The displayed `zeroResultQueries` list is capped at the ten most frequent zero-result groups by SQL, so summing it would understate the true number of zero-result rows. The rate instead divides by a dedicated `COUNT(*)` over all zero-result rows in the window (`zeroResultCount`), so even with many distinct empty queries the percentage stays accurate (`src/db/analytics.ts:42-46`, `src/db/analytics.ts:14-16`, `src/tools/analytics-tools.ts:32`).

## Branches and failure cases

- **No matching directory.** If the resolved directory does not exist, `resolveProject` throws before any query runs, and the tool call fails with `Directory does not exist: <path>` (`src/tools/index.ts:45-47`).
...
- **Some queries but no scored ones.** Average top score stays `null` and renders as `n/a` because the average ignores rows with a null `top_score` (`src/db/analytics.ts:32-34`).
...
- **List caps.** Each of the three lists is capped at ten entries by `LIMIT 10` in SQL, so the report never grows unbounded regardless of how many distinct queries were logged (`src/db/analytics.ts:38-58`).
