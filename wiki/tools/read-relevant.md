...
The tool is registered in `src/tools/search.ts:101` as the second tool inside `registerSearchTools`. The bulk of the work happens in `searchChunks` (`src/search/hybrid.ts:492`), which the handler calls and then formats into a single text block.
...
5. `searchChunks` embeds the query into a vector via `embed(query)` (`src/search/hybrid.ts:503`).
6. It runs two candidate fetches, each over-fetching `topK * 4` rows: a vector search (`db.searchChunks`) and a BM25 keyword search (`db.textSearchChunks`). The text search is wrapped in a `try/catch` that logs and falls back to vector-only if the FTS query throws (`src/search/hybrid.ts:505-512`).
7. The two result sets are fused by reciprocal-rank fusion in `mergeHybridScores`, chunks whose fused score is below `threshold` are dropped, and each surviving chunk is rescored by path type, filename affinity, and dependency fan-in (`src/search/hybrid.ts:515-558`).
8. If two or more chunks share the same parent chunk, `groupByParent` fetches the parent via `getChunkById` and replaces the siblings with the parent (`src/search/hybrid.ts:561`).
9. Before returning, `searchChunks` writes a `query_log` row recording the query, result count, the top vector hit's cosine similarity, top path, and elapsed milliseconds (`src/search/hybrid.ts:571`).
...
- **Threshold filtering.** Chunks scoring below `threshold` are dropped after fusion and before any rescoring (`src/search/hybrid.ts:516`). Because the fused score is a positional value compressed near `1` at the top and decaying down the list, a high threshold can produce zero results even when the index is populated; the default `0.3` is fairly permissive.
- **Keyword-search failure.** If the FTS5 query throws (for example on input that confuses the tokenizer despite `sanitizeFTS`), the `try/catch` logs a debug message and continues with vector-only results rather than failing the whole call (`src/search/hybrid.ts:507-512`).
- **Scope filter present vs absent.** With no scope fields, `buildFilter` returns `undefined` and the search is unfiltered. With a filter, the SQL over-fetches more candidate rows (`topK * FILTER_OVERFETCH`, where `FILTER_OVERFETCH` is `5`) before applying the path constraints, so filtering does not starve the result set. The filter clauses reach SQL as parametrized `LIKE ? ESCAPE '\'`, with each user-supplied extension and directory passed through `escapeLike` so a literal `%`, `_`, or `\` in a name does not act as a wildcard (`src/db/search.ts:36-77`).
...
- **More chunks than `top`.** Doc expansion can return slightly more than the requested `top` when documentation chunks would otherwise displace code (`src/search/hybrid.ts:564`).
