...
The tool is registered in `src/tools/search.ts:101` as the second tool inside `registerSearchTools`. The bulk of the work happens in `searchChunks` (`src/search/hybrid.ts:490`), which the handler calls and then formats into a single text block.
...
| `threshold` | number (0–1) | no | Minimum fused score for a merged chunk to survive. Defaults to `0.3` (`src/tools/search.ts:147`). |
...
| `query_log` row | One row inserted into the `query_log` table of the project's SQLite index for every call, recording the query text, result count, top vector score, top path, and elapsed milliseconds. Feeds analytics. |
...
sequenceDiagram
    ...
    Engine->>Engine: rrfFuse by rank, drop below threshold, rescore
...
5. `searchChunks` embeds the query into a vector via `embed(query)` (`src/search/hybrid.ts:501`).
6. It runs two candidate fetches, each over-fetching `topK * 4` rows: a vector search (`db.searchChunks`) and a BM25 keyword search (`db.textSearchChunks`). The text search is wrapped in a `try/catch` that logs and falls back to vector-only if the FTS query throws (`src/search/hybrid.ts:503`).
7. The two result sets are fused by reciprocal-rank fusion in `mergeHybridScores`, chunks whose fused score is below `threshold` are dropped, and each surviving chunk is rescored by path type, filename affinity, and dependency fan-in (`src/search/hybrid.ts:513`).
8. If two or more chunks share the same parent chunk, `groupByParent` fetches the parent via `getChunkById` and replaces the siblings with the parent (`src/search/hybrid.ts:559`).
9. Before returning, `searchChunks` writes a `query_log` row recording the query, result count, the raw top vector score, top path, and elapsed milliseconds (`src/search/hybrid.ts:567`).
...
Every chunk search records its own analytics row. Just before returning, `searchChunks` calls `db.logQuery(query, results.length, vectorResults[0]?.score ?? null, results[0]?.path ?? null, durationMs)` (`src/search/hybrid.ts:567`). Note that the logged "top score" is the **raw top vector cosine**, not the result's fused score — the fused score is a positional rank-fusion value that is always ≈1 at the top, so it would make the "avg top score" and the low-relevance (< 0.3) heuristic meaningless; the raw cosine preserves a real relevance signal (`src/search/hybrid.ts:564`). That facade method inserts into the `query_log` table — query text, result count, top score, top path, and duration in milliseconds, plus a timestamp (`src/db/analytics.ts:3`). When the result set is empty, top score and top path are stored as `null`, and `result_count` is `0`. This is the same table powering [search_analytics](search-analytics.md): zero-result and low-score rows are exactly how documentation and indexing gaps surface later. The write happens unconditionally on every call, including the empty-result case, because it lives inside `searchChunks` rather than the formatting code.
...
- **Threshold filtering.** Chunks scoring below `threshold` are dropped after fusion and before any rescoring (`src/search/hybrid.ts:514`). Because the fused score is a positional value compressed near `1` at the top and decaying down the list, a high threshold can produce zero results even when the index is populated; the default `0.3` is fairly permissive.
- **Keyword-search failure.** If the FTS5 query throws (for example on input that confuses the tokenizer despite `sanitizeFTS`), the `try/catch` logs a debug message and continues with vector-only results rather than failing the whole call (`src/search/hybrid.ts:508`).
...
- **More chunks than `top`.** Doc expansion can return slightly more than the requested `top` when documentation chunks would otherwise displace code (`src/search/hybrid.ts:562`).
...
[0.74] src/search/hybrid.ts:76-103  •  rrfFuse
export function rrfFuse(...)
...

---

[0.61] src/search/hybrid.ts:490-576  •  searchChunks
...
- `src/search/hybrid.ts` — `searchChunks` engine plus `rrfFuse`/`mergeHybridScores`: embedding, rank fusion, threshold cut, rescoring, parent grouping, doc expansion, and the `query_log` write.
...
- `src/config/index.ts` — defaults for `hybridWeight` (0.5) and `parentGroupingMinCount` (2).
