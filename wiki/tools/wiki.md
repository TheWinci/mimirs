...
Any other leading word falls through to a final `throw` that lists the valid commands, which the handler turns into a `failed:` string `src/wiki/rebuild.ts:1120`.
...
Bare `write` returns the top-level page-writing instructions. `write:page:<slug>` does the real assembly: it reads the prefetch snapshot (falling back to an empty one if `wiki/_prefetch.json` is absent), reads the plan, and calls `buildPagePacket` to bundle everything needed to write that one page — the page entry, its resolved flows, the dependency-map entries and annotations for its primary files, and all flow evidence `src/wiki/rebuild.ts:778-790`. It returns the kind-specific writing prompt — flow, screen, or one of the overview prompts, chosen by `writePagePrompt` `src/wiki/rebuild.ts:405-418` — followed by that bundle as a fenced JSON block `src/wiki/rebuild.ts:995-1016`.

### `validate-pages` — link check

`validate-pages` walks every `.md` file under `wiki/`, extracts each relative Markdown link, resolves it against the file's own directory, and records any that point at a missing file `src/wiki/rebuild.ts:726-741`. It skips absolute URLs, anchors, and links with a scheme such as `http:` `src/wiki/rebuild.ts:712-724`. The response either confirms all links resolve or lists each broken link `src/wiki/rebuild.ts:743-754`.
...
| `wiki/_prefetch.json` | Written by `shape`. The dependency map, index metadata, and grouped annotations used as evidence for the rest of the workflow `src/wiki/rebuild.ts:936`. |
...
| `.mimirs/wiki/*.md` | Written by `eject` / `eject:force` — copies of the ten packaged instruction files for per-project customization `src/wiki/rebuild.ts:1042-1043`. |
...
- **Unknown mode** — any leading word not handled by a branch hits the final `throw` listing the valid commands `src/wiki/rebuild.ts:1120`.
...
- **Empty index on `shape`** — when the index reports zero files, `shape` still writes the snapshot but prepends a stop notice pointing at [index_files](index-files.md) `src/wiki/rebuild.ts:394-403`.
- **Missing `wiki/_prefetch.json`** — `prefetch` reads it directly and throws if absent; `write:page` instead falls back to an empty prefetch snapshot so a page can still be drafted `src/wiki/rebuild.ts:1002-1004`.
...
- **`eject` collision** — without `force`, files already present in `.mimirs/wiki/` are skipped and listed; `eject:force` overwrites them `src/wiki/rebuild.ts:1038-1045`.
