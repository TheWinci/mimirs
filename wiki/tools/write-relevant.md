...
end-to-end inside `src/tools/search.ts:277-348`.
...
1. The caller invokes the tool with the `content` to add and optional `top`,
   ...
   characters; the rest are optional (`src/tools/search.ts:280-298`).
2. The handler resolves the project: `resolveProject` turns the optional
   ...
   back the `RagDB` and `RagConfig` (`src/tools/index.ts:21-37`). If the directory
...
3. It computes `topN = top ?? 3` and calls `searchChunks` with the *content* as the
   ...
   distinct files (`src/tools/search.ts:302-312`). The `threshold`,
...
8. Each surviving candidate is rendered as a one-block suggestion: its score, its
   ...
   text block (`src/tools/search.ts:333-346`).
...
signals (`src/search/hybrid.ts:63`, `src/config/index.ts:23`). The text side of the
...
(`src/tools/search.ts:320-331`). Requesting `topN * 3` chunks up front is what makes
...
trimmed (`src/tools/search.ts:338`). The idea is that the anchor is the tail of the
...
| --- | --- | --- | --- |
| `content` | string (1–5000 chars) | yes | The code or documentation you intend to add. Used directly as the search query (`src/tools/search.ts:281`). |
...
- **Directory does not exist.** `resolveProject` resolves the path and throws
  ...
  that error (`src/tools/index.ts:30-32`).
...
- **Fewer files than `topN`.** The best-per-file map may contain fewer entries than
  ...
  requested without an error (`src/tools/search.ts:329-331`).
