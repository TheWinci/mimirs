...
The optional `type` narrows results to one kind of symbol. It is a fixed enum at the tool boundary and maps directly to an `fe.type = ?` clause in SQL (`src/tools/search.ts:232-235`, `src/db/search.ts:269-272`).
...
- **No matching snippet chunk** — if no chunk's `entity_name` matches the symbol, `snippet` is `null` and the display omits the code preview (`src/db/search.ts:397`, `src/tools/search.ts:261`).
...
- `src/db/index.ts` — the `RagDB.searchSymbols` method that forwards to the query layer.
...
- `src/indexing/indexer.ts` — calls `upsertFileGraph` during indexing to populate exports from the extracted symbols.
