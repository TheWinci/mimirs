...
function writePagePrompt(slug: string, kind: string | undefined): string {
  ...
  return [
    ...
    "- Add a Mermaid sequence diagram with `autonumber` when the page describes a flow.",
    "- Add a numbered list below the diagram that explains each diagram step with more context.",
    "- Add an `Inputs` section when the page has `inputs`. Cover every item in that array, focusing on what the caller, environment, schedule, message, config, or file system provides to the flow.",
    "- Add an `Outputs` section when the page has `outputs`. Cover every item in that array, focusing on what the flow returns, writes, updates, starts, publishes, or otherwise changes.",
    ...
    "- Add `Branches and failure cases` when the code has empty-result paths, missing-input handling, optional flags, lock/query-only modes, startup phases, or error handling.",
...
function writeOverviewPagePrompt(slug: string, kind: string): string {
  ...
  return [
    ...
    diagramRequired
      ? "- Include at least one diagram. Use `graph` or `flowchart` for component/module/integration relationships, or `erDiagram` for data-model schemas. Sequence diagrams are usually wrong here; they belong on flow pages."
